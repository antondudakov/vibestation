...
- [x] Sessions sort by when they were last attached, most recent first, ties keeping tmux's order
- [x] A never-attached session sorts last and its age cell is empty
- [x] Age renders `now`, `5m`, `3h`, `12d` across the boundaries
- [x] Inside tmux, the current session is marked `▶` and every other attached session `●`
- [x] Outside tmux, `display-message` is never called and no row is marked `▶`
